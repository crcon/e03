  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 text-center px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full glass-dark mb-6 sm:mb-8 animate-fade-in">
          <Zap className="w-3 h-3 sm:w-4 sm:h-4 text-[#00b49d]" />
          <span className="text-xs sm:text-sm text-white/80 whitespace-nowrap">全球电网侧独立共享储能电站行业领导者</span>
        </div>

        {/* Main Title */}
        <h1
          ref={titleRef}
          className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold text-white mb-4 sm:mb-6 tracking-tight px-2"
        ...
        <h2
          ref={subtitleRef}
          className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-semibold text-[#00b49d] mb-4 sm:mb-6 opacity-0"
        >
          大储能生态
        </h2>
        ...
        <p
          ref={descRef}
          className="text-base sm:text-lg md:text-xl text-white/70 max-w-2xl mx-auto px-4 sm:px-0 opacity-0"
        >
          助力全国智慧能源电力时代转型
          <br className="hidden sm:block" />
          <span className="text-white/50 block sm:inline mt-1 sm:mt-0">深耕储能电力运营，以极致服务铸就客户全周期可持续价值</span>
        </p>
      </div>

      {/* Scroll Indicator */}
      <div className="absolute bottom-6 sm:bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
        <span className="text-white/40 text-xs sm:text-sm">向下滚动</span>
        <div className="w-5 h-9 sm:w-6 sm:h-10 rounded-full border-2 border-white/30 flex justify-center pt-2">
          <div className="w-1.5 h-2.5 sm:h-3 bg-[#00b49d] rounded-full animate-bounce" />
        </div>
      </div>
